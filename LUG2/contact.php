<?php 
    ...
    if($_SERVER["METHOD_REQUEST"] == "POST"){
        ...
        $customer_name = clearUserInput($_POST["customer_name"]);
        
        $customer_email = clearUserInput($_POST["customer_email"]); 
        $customer_email = Sanitize($customer_email, 'e'); 

        $contact_subject = clearUserInput($_POST["contact_subject"]);
        $contact_message = clearUserInput($_POST["contact_message"]); 

        $to = "algeserone@yandex.com"; 

        if(!userValidation($customer_name, 'e')){
            $errors["customer_name"] = "Empty Field"; 
        }else if (!preg_match("/^[a-zA-Z\s*']+$/", $customer_name)){
            $errors["customer_name"] = "Invalid String"; 
        }

        if(!userValidation($customer_email, 'e')){
            $errors["customer_name"] = "Empty Field"; 
        }else if (!userValidation($customer_email, 'm')){
            $errors["customer_name"] = "Invalid Email"; 
        }

        $header = "From:'$customer_email'.com \r\n";
        $header .= "Cc:'$to'.com \r\n";
        $header .= "MIME-Version: 1.0\r\n";
        $header .= "Content-type: text/html\r\n";

        mail($to, $contact_subject, $contact_message, $header); 
    }
